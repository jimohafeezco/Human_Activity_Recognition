...
def load_group(filenames, prefix=''):
    ...
    # TODO: stack the loaded variable into a 3d array, hint numpy dstack
    return loaded


def load_dataset_group(group, prefix=''):
    filepath = prefix + group + '/Inertial Signals/'
    # load all 9 files as a single arrayloa
    filenames = list()
    # total acceleration
...
def convert_sample(sample):
    ...
    :return:
    """
    converted = np.hstack((np.mean(sample, axis=0),np.std(sample, axis=0), np.max(sample, axis=0),np.min(sample, axis=0)))
    # TODO: convert that sample into an array of means,stds,max and mins
